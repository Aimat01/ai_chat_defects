...
1. **ПЕРВЫМ ДЕЛОМ** вызовите get_vehicle_data с license_plate для получения актуальных данных
   ...
   🔄 Если get_vehicle_data вернул found=False:
   - В ответе будет message с рекомендацией использовать MongoDB
   - Вызовите findDocuments с:
     * collection: "equipments"
     * query: {"license_plate_number": "{license_plate}"}
   - Извлеките базовые данные: brand, model, managers, project
   
   ✅ Если get_vehicle_data вернул found=True:
   - Используйте эти данные напрямую
   - Продолжайте со следующими шагами
...
**Формат ответа ЭТАП 2 (с VIN):**
{
    ...
    "auto_data": {
        ...
        "managers": ["Иванов И.И."],
        "project": "Проект А",
        "source": "postgresql или mongodb"  ← указать источник данных
    },
...
- Если get_vehicle_data вернул found=False, используйте findDocuments для получения данных из MongoDB
- Используйте реальные данные из БД для заполнения auto_data
...
- В auto_data всегда указывайте source ("postgresql" или "mongodb")
"""
